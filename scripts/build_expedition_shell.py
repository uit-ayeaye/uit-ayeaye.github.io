...
for p in paths:
    s=re.sub(r'(expedition-shell\.(?:css|js))\?v=[a-f0-9]+', r'\1', p.read_text());s=re.sub(r'<!-- expedition-shell:start -->.*?<!-- expedition-shell:end -->','',s,flags=re.S);s=s.replace(CSS,'').replace('</head>',CSS+'</head>');s=s.replace(JS,'').replace('</body>',DOCK+JS+'</body>')
    ...
    for asset in ['css/expedition-shell.css', 'js/expedition-shell.js']:
