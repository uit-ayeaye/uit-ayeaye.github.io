...
(async () => {
  ...
  assert.equal(await page.locator('.resume-dialog .resume-entry').count(), 6);
  await audit(page, 'Resume popup');
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('[data-resume]').first().evaluate(el => el === document.activeElement), true);
  ...
  console.log('PASS: filters, technology search/icons, artwork switch, personal collection controls, touch swipe, showcase tabs/arrows/source links, URL state, keyboard controls, mobile menu, theme/motion/Gear 5/layout persistence, all case routes, eight widths in both themes/layouts, no-JS fallback, and reduced motion.');
  await browser.close();
