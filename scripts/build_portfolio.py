...
def illustration(p):
    if p['image']:
        ...
        return f'<img class="project-image{cls}" src="{esc(p["image"])}" alt="{esc(p.get('preview_caption',p['title']))}" width="1440" height="1000" loading="lazy" decoding="async">'
    if p['id'] == 'som-bi':
...
def personal_collection():
    ...
    for i, (slug, name, role, desc) in enumerate(entries):
        ...
        cards += f'''<article class="personal-card"><div class="browser-frame"><div class="browser-bar"><span class="browser-dots" aria-hidden="true"><i></i><i></i><i></i></span><span>{domain}</span><span aria-hidden="true">↗</span></div><a class="personal-preview" href="/projects/{p['id']}/" {preview_attrs(p)}><img src="{p['image']}" alt="" width="1440" height="1000" loading="lazy"><span class="preview-personal-hint">Explore project ↗</span></a></div><div class="personal-copy"><p class="eyebrow">{role}</p><h3>{link(name, '/projects/'+slug+'/')}<span>0{i+1}</span></h3><div class="personal-links">{link('View case study ↗', '/projects/'+slug+'/', 'text-link')}{link('Visit website ↗', p['url'], 'text-link')}</div></div></article>'''
