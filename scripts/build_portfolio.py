...
def head(title, description, canonical, image='/images/og-thomas.jpg'):
    ...
    schema.update({'@id': 'https://thomasdlynn.dev/#person', 'image': 'https://thomasdlynn.dev/images/captain/thomas-captain.webp', 'email': 'mailto:hello@thomasdlynn.dev', 'knowsAbout': ['Software engineering', 'Game development', 'Web development', 'Mobile applications', 'Logistics software']})
...
def card(p, index):
    ...
    return f'''<article class="project-card" data-category="{esc(p['category'])}" data-search="{esc(terms.lower())}">{preview_cover(p, index)}<div class="project-body"><p class="project-eyebrow">{esc(p['category'])}</p><h3><a href="/projects/{p['id']}/">{esc(p['title'])}<span aria-hidden="true">↗</span></a></h3><p class="project-summary">{esc(p['summary'])}</p></div></article>'''
...
def personal_collection():
    ...
    for i, (slug, name, role, desc) in enumerate(entries):
        ...
        cards += f'''<article class="personal-card"><div class="browser-frame"><div class="browser-bar"><span class="browser-dots" aria-hidden="true"><i></i><i></i><i></i></span><span>{domain}</span><span aria-hidden="true">↗</span></div><a class="personal-preview" href="{p['image']}" {preview_attrs(p)}><img src="{p['image']}" alt="" width="1440" height="1000" loading="lazy"><span class="preview-personal-hint">Expand preview ⤢</span></a></div><div class="personal-copy"><p class="eyebrow">{role}</p><h3>{link(name, '/projects/'+slug+'/')}<span>0{i+1}</span></h3><div class="personal-links">{link('View case study ↗', '/projects/'+slug+'/', 'text-link')}{link('Visit website ↗', p['url'], 'text-link')}</div></div></article>'''
...
def build_project(p, i):
    ...
    gallery = ''
    if p.get('gallery'):
        gallery = '<p class="case-preview-note">Interface recreations based on the SOM BI source layouts. All names and metrics are synthetic sample data.</p><div class="system-gallery">'
        for item in p['gallery']:
            attrs = preview_attrs({**p, 'image': item['image'], 'title': 'SOM BI — ' + item['title']})
            gallery += f'<figure><a href="{item["image"]}" {attrs}><img src="{item["image"]}" alt="{esc(item["title"])} interface recreation with sample data" width="1440" height="1000" loading="lazy"></a><figcaption><strong>{esc(item["title"])}</strong><p>{esc(item["description"])}</p></figcaption></figure>'
        gallery += '</div>'
    signoff = '<div class="crew-signoff"><img src="/images/captain/thomas-captain.webp" alt="Thomas in a straw hat" width="1129" height="1393" loading="lazy"><div><strong>A page from my logbook.</strong><a href="/resume/" data-resume>Meet the builder · Open my CV ↗</a></div></div>'
