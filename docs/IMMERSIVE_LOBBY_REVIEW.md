...
## September 21 — mobile gestures, chart pins, and Creative Builder

Character changes now have one 280ms transition rather than competing legacy floats and a moving swipe stage. Gesture handling locks horizontal intent, verifies pointer identity, resets on cancellation/blur, and ignores the bubbling loss of an image's implicit touch capture when capture transfers to the stage. Vertical scrolling and pinch zoom remain native. Nearby hidden illustrations are warmed before the lobby enters view. Touch buttons skip the second JavaScript bounce, and world swipes exclude links, controls, and disclosures.

The three choices are raised chart pins with distinct compass, ship, and star marks, a glowing selected ring, steady 68px targets, and an immersive beacon that respects reduced motion and visibility. Updated the portfolio, résumé HTML/text, both PDFs, metadata, and versioned social cover to Creative Builder.

Validation: build and static verifier pass for 37 local route/asset/anchor pages and 44 SEO routes. Chrome touch emulation confirmed captain-to-ship swipe, cancelled swipe preserving ship, rapid choices ending on wanted, wanted-to-captain wraparound, and vertical finger scrolling preserving the selected character. Narrow 320px, 390px, 430px and desktop 1440px layouts checked; no page overflow in home, résumé, case study, gallery, or article templates. Grid/list differentiation, skill disclosure, personal collection, next world, navigation panel, résumé modal/title, keyboard End, and OS reduced motion checked. Both revised PDF first pages rendered and visually checked. No physical iOS/Android hardware test was performed.
