...
(() => {
 ...
 function reconcile(){
  ...
  videos.forEach(video=>{
   const eligible=allowed()&&(!touchDevice.matches||(video.id==='preview-loop'&&video.dataset.paused==='false'))&&video.dataset.paused!=='true'&&visible.has(video)&&!video.hidden&&!video.closest('[hidden]')&&(!modal||modal.contains(video))&&playing<(matchMedia('(max-width:700px)').matches?1:2);
 ...
 window.setVoyagePreview=(src,poster)=>{
  ...
  video.dataset.src=src||'';if(touchDevice.matches)video.dataset.paused='true';else delete video.dataset.paused;video.hidden=!src;video.poster=poster||'';register(video);reconcile();
 };
 ...
 connection?.addEventListener('change',reconcile);touchDevice.addEventListener('change',reconcile);
